...
// DownloadFile downloads a Telegram file by its file_id and saves it to the
// specified local path. It calls getFile to obtain the remote path, then
// streams the content to disk.
func (c *Client) DownloadFile(ctx context.Context, fileID, destPath string) error {
	resp, err := c.Call(ctx, "getFile", map[string]any{"file_id": fileID})
	if err != nil {
		return fmt.Errorf("getFile: %w", err)
	}
	if !resp.Ok {
		return fmt.Errorf("getFile: %s", resp.Description)
	}
	var info struct {
	...
	dlURL := "https://api.telegram.org/file/bot" + c.token + "/" + info.FilePath
