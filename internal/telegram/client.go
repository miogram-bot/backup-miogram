...
// GetFile returns the file info for a given file_id.
func (c *Client) GetFile(ctx context.Context, fileID string) (FileInfo, error) {
	...
	if !resp.Ok {
		return FileInfo{}, fmt.Errorf("getFile: %s", resp.Description)
	}
	var info FileInfo
...
// DownloadFile downloads a file by its file_path and returns the raw bytes.
...
// FileInfo represents the result of a getFile call.
